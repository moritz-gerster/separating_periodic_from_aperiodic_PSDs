...
def calc_PSDs(seiz_data, pre_seconds, post_samples, seiz_len_samples, srate,
              **welch_params):
    """Calc PSDs."""
    # n_freq = srate // 2 + 1
    ...
    data_pre = seiz_data[pre_slice]
    data_seiz = seiz_data[seiz_slice]
    data_post = seiz_data[post_slice]
...
EEG_psds = calc_PSDs(seiz_data, pre_seconds, post_samples, seiz_len_samples, srate, **welch_params)
...
rect_min = seiz_data.min() * 1.1
rect_height = np.abs(seiz_data).max() * 2
...
# Plot EEG seizure
ax = axes[0, 0]
ax.plot(time_EEG, seiz_data, c=c_empirical, lw=1)
