...
def input_psd(ax):
    """Plot input PSD."""
    ax.loglog(freqs_fooof, psd_full_fooof, c_sim, lw=lw_PSD, label="PSD")
    ax.set(xticks=[], yticks=[], xticklabels=[], yticklabels=[])
...
def fooof_1(ax, ybottom=1):
    ...
    plot_spectrum(fm.freqs, 10**fm.power_spectrum, log_freqs=False,
                  lw=lw_fooof, color=c_sim, ax=ax)
    plot_spectrum(fm.freqs, 10**init_ap_fit, log_freqs=False,
                  label='Initial Fit', color=c_fit, lw=lw_fit, alpha=1,
                  ls=ls_fit, ax=ax)
    ax.grid(False)
    ax.set_yscale("log")
    ax.set_xscale("log")
...
def IRASA_res1(ax, ybottom=None, ytop=None):
    ...
    ax.loglog(freqs_irasa, psds_up[0], c_h1, lw=lw_IR)
    ax.loglog(freqs_irasa, psd_full_irasa, c_sim, lw=lw_PSD, ls="--",
              label=rf"$h={hset[0]:.1f}$")
    ax.loglog(freqs_irasa, psds_dw[0], c_h1, lw=lw_IR,
...
def IRASA_res2(ax, ybottom=None, ytop=None):
    ...
    ax.loglog(freqs_irasa, psd_full_irasa, c_sim, lw=lw_PSD, ls="--")
    ax.loglog(freqs_irasa, psds_up[1], c_h2, lw=lw_IR,
              label=rf"$h={hset[1]:.1f}$")
    ax.loglog(freqs_irasa, psds_dw[1], c_h2, lw=lw_IR,
...
def IRASA_res3(ax, ybottom=None, ytop=None):
    ...
    ax.loglog(freqs_irasa, psds_up[2], c_h3, lw=lw_IR,
              label=rf"$h={hset[2]:.0f}$")
    ax.loglog(freqs_irasa, psd_full_irasa, c_sim, lw=lw_PSD, ls="--")
    ax.loglog(freqs_irasa, psds_dw[2], c_h3, lw=lw_IR,
...
def IRASA_mean1(ax, ybottom=None, lw_median=.1):
    ...
    ax.loglog(freqs_irasa, psd_full_irasa, c_sim, lw=lw_PSD, ls="--")
    ax.loglog(freqs_irasa, psds_resampled[0], c_h1, lw=lw_IR,
              label=f"h={hset[i]}")
    ax.spines["right"].set_visible(False)
...
def IRASA_mean2(ax, ybottom=None, lw_median=.1):
    ...
    ax.loglog(freqs_irasa, psd_full_irasa, c_sim, lw=lw_PSD, ls="--")
    ax.loglog(freqs_irasa, psds_resampled[1], c_h2, lw=lw_IR,
              label=f"h={hset[i]}")
    ax.spines["right"].set_visible(False)
    ax.spines["left"].set_visible(False)
    ax.spines["top"].set_linewidth(lw_median)
...
def IRASA_mean3(ax, ybottom=None, lw_median=.1):
    ...
    ax.loglog(freqs_irasa, psd_full_irasa, c_sim, lw=lw_PSD, ls="--")
    ax.loglog(freqs_irasa, psds_resampled[2],  c_h3, lw=lw_IR,
              label=f"h={hset[i]}")
...
def make_frame(ax, c, title=None, lw_box=0, **kwargs):
