...
signal_on = signal_on.get_data(start=start, stop=stop,
                               reject_by_annotation="nan")[ch]
signal_off = signal_off.get_data(start=start, stop=stop,
                                 reject_by_annotation="nan")[ch]
...
for i in range(9):
    ...
    # IRASA
    IRASA_real_on = irasa(data=signal_on, **irasa_params)
    IRASA_real_off = irasa(data=signal_off, **irasa_params)
    IRASA_sim_on = irasa(data=signal_sim_on, **irasa_params)
    IRASA_sim_off = irasa(data=signal_sim_off, **irasa_params)
    ...
    errs_f_sim.append(error_fooof_sim)
    errs_f_real.append(error_fooof)
    ...
    errs_i_sim.append(error_IRASA_sim)
    errs_i_real.append(error_IRASA)
...
data = {"freq_ranges": freq_ranges,
        "err_f_sim": errs_f_sim, "err_i_sim": errs_i_sim,
        "err_f_real": errs_f_real, "err_i_real": errs_i_real}
df = pd.DataFrame(data)
save_name = "df_sim_real.pkl"
df.to_pickle(save_name)


fig, ax = plt.subplots(1, 1)
for i in range(df.shape[0]):
    ax.plot(df["freq_ranges"][i], [df["err_f_sim"][i], df["err_f_sim"][i]],
            "r--", alpha=1, label="fooof")
    ax.plot(df["freq_ranges"][i], [df["err_i_sim"][i], df["err_i_sim"][i]],
            "b:", alpha=1, label="IRASA")
ax.set_xlabel("Frequency Range [Hz]")
ax.set_ylabel("Fitting error ground truth")
...
plt.tight_layout()
plt.savefig("sim_summary.pdf", bbox_inches="tight")
plt.show()


fig, ax = plt.subplots(1, 1)
for i in range(df.shape[0]):
    ax.plot(df["freq_ranges"][i], [df["err_f_real"][i], df["err_f_real"][i]],
            "r--", alpha=1, label="fooof")
    ax.plot(df["freq_ranges"][i], [df["err_i_real"][i], df["err_i_real"][i]],
            "b:", alpha=1, label="IRASA")
ax.set_xlabel("Frequency Range [Hz]")
ax.set_ylabel("Fitting error estimation")
...
ax.legend(handles[:2], labels[:2])
plt.tight_layout()
plt.savefig("real_summary.pdf", bbox_inches="tight")
plt.show()
...
# Artefakten wenn der Frequenz Bereich sehr niedrig beginnt. Bei kurzem
# Frequenzbereich gibt es seltsamerweise bessere Ergebnisse mit kleinem
# hset_max.
