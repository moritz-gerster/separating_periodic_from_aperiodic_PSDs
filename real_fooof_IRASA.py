...
# Colors
c_on = B_config.plot_params["c_on"]
c_off = B_config.plot_params["c_off"]
...
freq_ranges = [[1, 45], [2, 45], [5, 45], [10, 45], [15, 45], [20, 45],
               [30, 45], [35, 45], [40, 45], [40, 60]]

best_fooof_params = [{"peak_width_limits": (1, 12), "peak_threshold": 1}]
best_IRASA_params = [{"hset_max": 1.9, "win_sec_i": .5, "num": 5}]

freq_range = freq_ranges[i]

# Normalize power offset
low = np.where(frequencies == freq_range[0])[0][0]

psd_on = psd_on / psd_on[low]
psd_off = psd_off / (psd_off[low] * 10)
psd_sim_on = psd_sim_on / psd_sim_on[low]
psd_sim_off = psd_sim_off / (psd_sim_off[low] * 10)
psd_sim_on_ap = psd_sim_on_ap / psd_sim_on_ap[low]
psd_sim_off_ap = psd_sim_off_ap / (psd_sim_off_ap[low] * 10)
#psd_sim_on_osc = psd_sim_on_osc / psd_sim_on_osc[low]
#psd_sim_off_osc = psd_sim_off_osc / (psd_sim_off_osc[low] * 10)
...
# Fit Params
peak_width_min = 1
fooof_params = {"peak_width_limits": (1, 12),
#                "max_n_peaks": np.inf,
#                "min_peak_height": 0,
                "peak_threshold": 1}#,
#                "aperiodic_mode": "fixed"}
# hset_max = srate / 8 / freq_range[1]
hset_max = 1.9
win_sec_i = .5
num = 5
irasa_params = {"sf": srate, "band": freq_range,
                "kwargs_welch": {'average': 'median'},
                "win_sec": win_sec_i,
                "hset": np.linspace(1.1, hset_max, num=num)}
...
fm_on_sim.plot(plt_log=True, ax=ax,
               model_kwargs={"color": c_on, "linestyle": "--"},
               data_kwargs={"alpha": 0})
...
fm_off_sim.plot(plt_log=True, ax=ax,
                model_kwargs={"color": c_off, "linestyle": "--"},
                data_kwargs={"alpha": 0})
...
error_fooof = (abs(slope_truth_on - exponent_on) +
               abs(slope_truth_on - exponent_sim_on) +
               abs(slope_truth_off - exponent_off) +
               abs(slope_truth_off - exponent_sim_off))
ax.set_title(f"Fooof estimated error: {error_fooof:.2f}")
...
ax.plot(fm_off_sim.freqs, fooof_osc_sim_off-.8, "--", c=c_off, label="osc sim off")
...
error_IRASA = (abs(slope_truth_on - slope_on) +
               abs(slope_truth_on - slope_sim_on) +
               abs(slope_truth_off - slope_off) +
               abs(slope_truth_off - slope_sim_off))
ax.set_title(f"IRASA estimated error: {error_IRASA:.2f}")

ax.semilogy(*ir_on, label=label_on)
ax.semilogy(*ir_ap_on, label="aperiodic")
ax.semilogy(*ir_off, label=label_off)
ax.semilogy(*ir_ap_off, label="aperiodic off")
ax.semilogy(*ir_sim_on, c=c_on, label=label_sim_on)
ax.semilogy(*ir_ap_sim_on, c="darkgreen", label="aperiodic sim on")
ax.semilogy(*ir_sim_off, c=c_off, label=label_sim_off)
ax.semilogy(*ir_ap_sim_off, c="darkgreen", label="aperiodic sim off")
ax.semilogy(*p_fit_on, label="Aperiodic fit")
...
ax.plot(freq, osc_off-1, c_off, label="osc off")
ax.plot(freq, osc_sim_on, "--", c=c_on, label="osc sim")
ax.plot(freq, osc_sim_off-1, "--", c=c_off, label="osc sim")
...
plt.suptitle(f"Fit Range: {freq_name}")
...
# =============================================================================
# DELETE
# freq = np.fft.fftfreq(signal_on[:10000].size, 1/srate)
# psd = np.fft.fft(signal_on[:10000])
# 
# mask = (freq < 100) & (freq > 0)
# plt.plot(freq[mask], abs(psd[mask]))
# plt.show()
# plt.loglog(freq[mask], abs(psd[mask]))
# plt.show()
# 
...
# plt.show()
# 
# plt.plot(freq[mask], psd[mask].imag)
# plt.show()
# plt.loglog(freq[mask], psd[mask].imag)
# plt.show()
# =============================================================================
