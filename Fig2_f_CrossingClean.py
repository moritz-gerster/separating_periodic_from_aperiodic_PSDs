...
ax.hlines(.8e-7, 4, 100, color=c_range1, ls="--", label="Fit range 1")
ax.hlines(2.1e-8, 11, 100, color=c_range2, ls="--", label="Fit range 2")
ax.hlines(3.9e-10, 23, 100, color=c_range3, ls="--",  label="Fit range 3")
ax.text(s="Fitting range: 4-100Hz", y=1.1e-7, **anno_dic)
ax.text(s="11-100Hz", y=3e-8, **anno_dic)
ax.text(s="23-100Hz", y=5e-10, **anno_dic)
...
ax.hlines(.9, 11, 100, color=c_range2, ls="--")
ax.hlines(.8, 23, 100, color=c_range3, ls="--")
...
xticks = [1, 10, 100, 600]
ax.set(xlabel="Frequency [Hz]", ylabel=r"PSD [$\mu$$V^2$/Hz]", xticks=xticks,
       xticklabels=xticks)
...
arrow_dic = dict(arrowprops=dict(arrowstyle="->, head_length=0.2,head_width=0.2"))
...
leg = ax_leg.legend(handles, labels, ncol=3, frameon=True, fontsize=12,
                    labelspacing=0.1, bbox_to_anchor=(.9, 1.1))
