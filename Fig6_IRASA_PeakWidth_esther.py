...
def annotate_fit_range(ax, xmin, xmax, height, ylow=None, yhigh=None,
    ...
    if annotate_range:
        ...
        ax.text(text_pos, text_height, s=f"{xmax-xmin:.1f}Hz", **anno_dic)
    if ylow and yhigh:
...
# %% Plot
fig = plt.figure(figsize=[width, 3.9], constrained_layout=True)

gs0 = gridspec.GridSpec(2, 1, figure=fig, height_ratios=[1, 10])


gs02 = gridspec.GridSpecFromSubplotSpec(1, 1, subplot_spec=gs0[0])
ax_leg = fig.add_subplot(gs02[0])
ax_leg.axis("off")

gs03 = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs0[1])
