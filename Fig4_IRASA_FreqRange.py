...
for low in lower_fitting_borders:
    ...
    print(f"{i/len(lower_fitting_borders)*100:.2f}%")
    i += 1
...
for h_max in h_maxima:
    ...
    plot_IRASA = (freq_IR, 10**IR_fit, c_IRASA)
    ...
    plot_IRASA_eff = (freq_IR_eff, 10**IR_fit_eff, c_IRASA)
...
plot_IRASA_low = (freq_low, 10**IR_fit_low, c_IRASA)
plot_IRASA_high = (freq_high, 10**IR_fit_high, c_IRASA)
plot_IRASA_low_eff = (freq_low_eff, 10**IR_fit_low_eff, c_IRASA)
...
plot_IRASA_eff_low = (freq_IR_eff_low, 10**IR_fit_eff_low, c_IRASA)
plot_IRASA_eff_high = (freq_IR_eff_high, 10**IR_fit_eff_high, c_IRASA)
plot_IRASA_eff_low_eff = (freq_IR_eff_low_eff, 10**IR_fit_eff_low_eff, c_IRASA)
...
ylabel_a1 = "PSD [a.u.]"
labelpad = 5
axes_a1 = dict(xticklabels=xticklabels_a1, xlim=xlim_a,
               yticklabels=yticklabels_a1)
...
axes_b = dict(xticks=xticks_b, xlabel=xlabel_b, xticklabels=xticks_b,
              ...
              xlim=xlim_b)
tiny_leg = dict(borderaxespad=0, labelspacing=.3, handlelength=1.5,
                handletextpad=.5)  # borderpad=.1)
...
def annotate_fit_range(ax, xmin, xmax, ylow, yhigh, height,
    ...
    anno_dic = dict(ha=ha, va="center", bbox=dict(fc="white", ec="none",
                    alpha=box_alpha), fontsize=annotation_fontsize)
...
ax.set_yticklabels(yticklabels_a1, minor=True)
...
ax.loglog(*plot_IRASA_low, lw=lw)
ax.loglog(*plot_IRASA_eff_low, alpha=.5, lw=lw, label=label_low)
