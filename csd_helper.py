...
def bootstrap_coherence_diff(x, y, nperseg, mag=True, imag=True,
        N_bootstrap=1000, fs=1.0, frange=(None, None), axis=-1, **kwargs):
    ...
    Args:
        ...
        frange (tuple): a tuple of frequencies between which the test is
            calculated (including these frequencies), if None the corresponding
            limit is set to 0 (for the first value) or Nyquist (for the second)
    ...
    if frange[0] == None:
        frange = (0, frange[1])
    if frange[1] == None:
        frange = (frange[0], fs/2. + 1) # just add 1 to not loose the
        # largest frequency due to floating point round-off errors
    ...
    f, y_t, y_csd = scipy.signal.spectral._spectral_helper(y[idx1], y[idx2],
            fs=fs, nperseg=nperseg, axis=-1, mode='psd', **kwargs)
    f_mask = np.all([f >= frange[0], f <= frange[1]], 0)
    f = f[f_mask]
    x_csd = x_csd[...,f_mask, :]
    y_csd = y_csd[...,f_mask, :]
    # reject temporal segments with NaNs of infs
# =============================================================================
#     CHANGE: NP.ALL LEAD TO DIVISION BY ZERO ERROR IF A BAD CHANNEL IS ALL NAN
#     -> CHANGE NP.ALL TO NP.ANY
...
# =============================================================================
    x_csd = x_csd[...,np.any(np.isfinite(x_csd),
        axis=tuple(range(x_csd.ndim - 1)))]
    y_csd = y_csd[...,np.any(np.isfinite(y_csd),
        axis=tuple(range(y_csd.ndim - 1)))]
    if (mag and imag):
    ...
    for _ in trange(N_bootstrap):
        ...
        if (mag and imag):
            ...
            t_boot.append(_jackknife_coherence_diff(
                    ...
                    mag, imag, z_transform=True))
    if (mag and imag):
        return f, (t_mag, np.array(t_mag_boot)), (t_imag, np.array(t_imag_boot))
    else:
        return f, (t, t_boot)
