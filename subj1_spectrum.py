...
ch_names = ['SMA', 'leftM1', 'rightM1',
            'STNR01', 'STNR12', 'STNR23',
            'STNL01', 'STNL12', 'STNL23']
srate = 2400
...
on_data = np.load(f_on_data)[:n_channels]
off_data = np.load(f_off_data)[:n_channels]
...
ax.set_ylim([0, 1])
ax.set_xlim([0,45])
ax.set_title(r'absolute imaginary coherence channel {0} vs. nan channel'.format(
    ch_names[0]))
...
fig = plt.figure()
ax = fig.add_subplot(111)
ax.plot(on_f, np.abs(on_coherence.imag)[0,2], 'r-', label='ON')
ax.plot(off_f, np.abs(off_coherence.imag)[0,2], 'b-', label='OFF')
ax.scatter(f_stat[imag_p[0,2]<0.05], 0.9*np.ones(np.sum(imag_p[0,2]<0.05)),
    edgecolors='k', facecolors='k')
ax.set_ylim([0, 1])
ax.set_xlim([0,45])
...
# ax[0].set_yticklabels(["SMA"] + ch_names)
...
# fig = plt.figure()
# ax = fig.add_subplot(111)
# ax.plot(on_f, np.abs(on_coherence)[0,5], 'r-', label='ON')
# ax.plot(off_f, np.abs(off_coherence)[0,5], 'b-', label='OFF')
# ax.scatter(f_stat[mag_p[0,5]<0.05], 0.9*np.ones(np.sum(mag_p[0,5]<0.05)),
#     edgecolors='k', facecolors='k')
# ax.set_ylim([0, 1])
# ax.set_xlim([0,45])
...
# ax.set_xlim([0,45])
# ax.set_title(r'magnitude coherence channel {0} vs. {1}'.format(
#     ch_names[0], ch_names[1]))
# ax.set_xlabel('frequency (Hz)')
# ax.legend()
# fig.savefig('mag_01_coherence_0_5.png')
# 
# fig = plt.figure()
# ax = fig.add_subplot(111)
# ax.plot(on_f, np.abs(on_coherence.imag)[0,5], 'r-', label='ON')
# ax.plot(off_f, np.abs(off_coherence.imag)[0,5], 'b-', label='OFF')
# ax.scatter(f_stat[imag_p[0,5]<0.05], 0.9*np.ones(np.sum(imag_p[0,5]<0.05)),
#     edgecolors='k', facecolors='k')
# ax.set_ylim([0, 1])
# ax.set_xlim([0,45])
# ax.set_title(r'absolute imaginary coherence channel {0} vs. {1}'.format(
